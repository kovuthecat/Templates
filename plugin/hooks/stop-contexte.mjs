...
for (const ref of revuesManquantes(cwd, repereHead)) {
  problemes.push(
    ...
    `PLAN (/fin-de-tache, « Relecture de session ») — c'est LUI qui dépose le fichier, et une revue ` +
    `lancée en arrière-plan comme dernier geste tourne pour rien, son retour arrivant dans un tour ` +
    `que plus personne ne lit. Si tu viens au contraire de la verser dans \`TASKS.md\` au tri de ` +
    `clôture : c'est le repère \`Revues: ${ref}\` qui manque au commit du tri (/fin-de-tache ` +
    `point 16) — le \`.revue.md\` n'étant jamais commité, ce repère est la seule trace qu'il a existé.`
  );
}
