...
# Appendix F — Verifications du 2026-09-15

Entrées locales, vérifiées le 2026-09-15 contre la documentation officielle citée, pour des
garanties dont une règle du workflow dépendait. Une entrée = capacité · fait vérifié le 2026-09-15
· source officielle · règle locale concernée · décision.

| Capacité | Fait vérifié le 2026-09-15 | Source officielle | Règle locale concernée | Décision |
|---|---|---|---|---|
| `allowed-tools` (frontmatter de skill) | Pré-autorise les outils listés pour le tour qui invoque la skill ; ne retire rien — vérifié le 2026-09-15. | Skills — <https://code.claude.com/docs/en/skills> | `/cadrer`, `/revue-de-conception` (`plugin/EXECUTANT.md`, P5/S1/T1) | conserver, reformulé |
| `disallowed-tools` (frontmatter de skill) | Retire les outils listés pour le seul tour qui invoque la skill ; se réinitialise au message suivant ; ne peut retirer `EndConversation` seul — vérifié le 2026-09-15. | Skills — <https://code.claude.com/docs/en/skills> | `/cadrer`, `/revue-de-conception` (P5/S1/T1) | conserver, ajouté |
| `effort` (frontmatter d'agent) | Documenté, valeurs `low…max`, défaut « inherits from session » ; affiché par `/tasks` à partir de la version ≥ 2.1.242 ; **non sondé localement** sur ce poste — vérifié le 2026-09-15 sur la doc seule, sonde runtime prévue P5/S5. | Subagents — <https://code.claude.com/docs/en/sub-agents> | `WORKFLOW.md` §3 (« l'outil `Agent` pose le modèle, jamais l'effort »), agent `critique-plan` | différer — sonde P5/S5 |
